...
\begin{tikzpicture}[node distance=1.5cm, auto]
    % Node definitions
    \node [block] (user) {Client App (Next.js)};
    \node [cloud, above of=user, yshift=0.5cm] (firebase) {Firebase Auth};
    \node [block, right of=user, xshift=2.5cm] (gateway) {FastAPI Gateway};
    \node [process, below of=gateway, yshift=-0.5cm] (parser) {PDF Parser (PyMuPDF)};
    \node [process, right of=parser, xshift=2.5cm] (embed) {all-MiniLM-L6};
    \node [database, above of=embed, yshift=0.5cm] (faiss) {Isolated FAISS};
    \node [block, right of=gateway, xshift=2.5cm] (router) {Inference Router};
    \node [cloud, above of=router, yshift=0.5cm] (gemini) {Gemini Cloud API};
    \node [process, below of=router, yshift=-0.5cm] (ollama) {Local Ollama (Ngrok)};
